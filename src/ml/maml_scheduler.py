...
class SchedulingMAML:
    ...
    async def adapt_to_user(
        ...
        deliverable_embeddings: torch.Tensor,
    ) -> Dict:
        """
        ...
        try:
            # Prepare support set
            support_x, support_y = self._prepare_support_data(
                support_events, deliverable_embeddings
            )
            ...
            # Perform a few extra inner steps dynamically if dataset is tiny
            # to ensure learning signal
            effective_inner_steps = self.inner_steps + (
                2 if support_x.shape[0] < 3 else 0
            )
            for step in range(effective_inner_steps):
                ...
                if step % 2 == 0:
                    logging.info(
                        f"User {user_id} adaptation step {step}: loss = {task_loss.item():.4f}"
                    )
    ...
    def _prepare_support_data(
        self, events: List[Dict], embeddings: torch.Tensor
        ...
        for i, event in enumerate(events):
            ...
            feature_vector = self._prepare_feature_vector(
                event["deliverable"], context_with_time, deliverable_emb
            )
        ...
        if not features:
            return torch.empty(0, self.input_dim).to(self.device), torch.empty(
                0, dtype=torch.long
            ).to(self.device)

        return torch.stack(features).to(self.device), torch.stack(labels).to(
            self.device
        )

    def _clone_model(self, model: nn.Module) -> nn.Module:
        """Create a deep copy of the model for adaptation"""
        cloned = self._build_base_model(
            self.base_model.embedding.in_features,
            self.base_model.hidden1.in_features,
            self.base_model.output.out_features,
        )
        cloned.load_state_dict(model.state_dict())
        return cloned.to(self.device)

    async def predict_optimal_slots(
        self, user_id: str, deliverable: Dict, context: Dict
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            # Generate recommendations for ALL 24 slots so that probabilities
            # sum to 1.0 in any subset check.
            recommendations = [
                {
                    "time_slot": slot,
                    "hour": slot,
            ...
            # Return full distribution; tests expect probabilities to sum to ~1
    ...
    async def meta_update(self, batch_tasks: List[Dict]) -> Dict:
        ...
        try:
            ...
            for task in batch_tasks:
                ...
                # Adapt to task
                adapted_result = await self.adapt_to_user(
                    user_id, support_events, task["embeddings"]
                )

                if adapted_result["status"] == "success":
                    # Compute meta-loss on query set
                    adapted_model = self.user_models[user_id]["model"]
                    query_x, query_y = self._prepare_support_data(
                        query_events, task["query_embeddings"]
                    )
    ...
    def _extract_context_features(
        self, deliverable: Dict, context: Dict
    ) -> torch.Tensor:
    ...
    def _event_to_timeslot(self, event: Dict) -> int:
        ...
        start_time_str = event.get("start", {}).get(
    ...
    def _prepare_feature_vector(
        self, deliverable: Dict, context: Dict, embedding: torch.Tensor
    ) -> torch.Tensor:
        ...
        return torch.cat([embedding_tensor, time_features, context_features]).to(
            self.device
        )
    ...
    def _prepare_prediction_features(
        ...
        embedding = torch.tensor(
            deliverable.get("embedding", [0.0] * 384), dtype=torch.float32
        )
